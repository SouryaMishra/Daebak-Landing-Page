...
navToggle.addEventListener("change", e => {
    e.target.checked ? document.body.classList.add("nav-open") : document.body.classList.remove("nav-open");
    document.querySelectorAll(".links a").forEach(anchor => e.target.checked ? anchor.classList.add("slide-from-left") : anchor.classList.remove("slide-from-left"));
})
...
navLinks.forEach(navLink => navLink.addEventListener("click", function() {
    document.querySelectorAll(".side-nav-link-item .nav-link.active").forEach(el => el.classList.remove("active"));
    this.classList.add("active");
}))

copyIcons.forEach(copyIcon => copyIcon.addEventListener("click", function(e){
    e.preventDefault();
