...
document.querySelector(".btn-slider.left").addEventListener("click", () => {
  if (dx >= 5) return;
  dx++;
  document.querySelector(".carousel").style.transform = `translateX(calc(-${
    dx * 100
  }% - ${dx * 32}px))`;
  document
...
const rotateCard = (entries) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      document
        .querySelectorAll(".languages .card")
...
const infoContentObserver = new IntersectionObserver(slideX, {
  threshold: 0.3,
});
